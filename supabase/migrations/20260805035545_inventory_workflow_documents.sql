...
begin
  ...
  for line in select * from public.inventory_document_lines where document_id = doc.id loop
    ...
    update public.inventory_products set stock = after_value, status = case when after_value <= 0 then 'Out of Stock' when after_value <= 100 then 'Low Stock' when after_value >= 500 then 'High Stock' else 'In Stock' end, updated_by = p_actor_id, updated_at = now() where sku = line.sku;
    insert into public.inventory_movements (document_id, line_id, sku, direction, quantity, before_stock, after_stock, business_date, actor_id, actor_name) values (doc.id, line.id, line.sku, direction_value, line.quantity, before_value, after_value, doc.business_date, p_actor_id, coalesce(p_actor_name, ''));
    insert into public.inventory_activity (sku, product_name, action, quantity_label, detail, changes, actor_id, actor_name) values (line.sku, coalesce(line.product_name, product.name), case when direction_value = 'out' then 'OUT' else 'IN' end, case when direction_value = 'out' then '-' else '+' end || line.quantity::text, case when direction_value = 'out' then '出货单入账 ' else '入库单入账 ' end || doc.doc_no, '库存：' || before_value::text || ' → ' || after_value::text, p_actor_id, coalesce(p_actor_name, ''));
